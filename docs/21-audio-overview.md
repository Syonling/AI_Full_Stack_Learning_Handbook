...
::: tip 🛠 21~25 五章的跟做方式（重要，只说一次）
音频项目复杂度高，不要求像 07 章那样"全自建"，但**禁止复制粘贴**：
每段代码**先读懂、再手敲**，敲完立刻跑对应的验收标准，通过了才前进。
手敲时你会自然地问"这行为什么这么写"——那正是学习发生的时刻。
想要更高强度？每节先只看开头的目标描述，自己试 20 分钟再看参考实现。
:::
